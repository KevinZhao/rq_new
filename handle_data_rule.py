...
class Handle_data_df(Handle_data_rule):
    ...
    def before_trading_start(self, context):
        ...
        for stock in context.stock_list:
            #初始化30分钟线数据
            context.bar_30[stock] = pd.DataFrame(history_bars(stock, 150, frequency = '30m', fields = None), index = None)
            context.bar_60[stock] = pd.DataFrame(history_bars(stock, 150, frequency = '60m', fields = None), index = None)
            context.bar_15[stock] = pd.DataFrame(history_bars(stock, 150, frequency = '15m', fields = None), index = None)
        ...
        #次新指数
        context.index_df = pd.DataFrame(history_bars('399678.XSHE', 150, frequency = '1d', fields = None), index = None)
    ...
    def handle_data(self,context,data):
        ...
        if context.timedelt % 30 == 0:
            ...
            temp_data = pd.DataFrame(history_bars('399678.XSHE', 1, frequency = '1d', fields = None, include_now = True), index = None)
    ...
    def handle_minute_data(self, context, data, stock):
        ...
        if context.timedelt % 15 == 0:
            temp_data = pd.DataFrame(history_bars(stock, 1, frequency = '15m', fields = None, include_now = True), index = None)
        ...
        if context.timedelt % 30 == 0:
            ...
            temp_data = pd.DataFrame(history_bars(stock, 1, frequency = '30m', fields = None, include_now = True), index = None)
        ...
        if context.timedelt % 60 == 0:
            ...
            temp_data = pd.DataFrame(history_bars(stock, 1, frequency = '60m', fields = None, include_now = True), index = None)
